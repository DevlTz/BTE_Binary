...
// Pretty-printing the tree in infix notation with parentheses
    ...
    std::unique_ptr<Node> parse_calculator_expression(const std::vector<std::string>& tokens) {
        ...
        for (const auto& t : tokens) {
            if (is_operator(t)) {
                ...
                auto opnode = std::make_unique<Node>(t[0], std::move(left), std::move(right)); //Gets the first character of the operator string 
